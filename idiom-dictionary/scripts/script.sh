#!/usr/bin/env bash
# idiom-dictionary - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${IDIOM_DICTIONARY_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/idiom-dictionary}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
idiom-dictionary v$VERSION

Multi-purpose utility tool

Usage: idiom-dictionary <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "idiom-dictionary v$VERSION" ;;
